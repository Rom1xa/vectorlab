...
Vector::Vector(const ValueType* rawArray, const size_t size, float coef) {
    ...
    for (int i = 0; i < size; i++) { _data[i] = rawArray[i]; }
}
...
double Vector::loadFactor() const { return static_cast<double>(_size) / static_cast<double>(_capacity); }
...
void Vector::shrinkToFit() { reserve(_size); }
